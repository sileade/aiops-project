...
from app.services import ai_service, system_service, analysis_service
from app.services.proxmox_service import ProxmoxService
from app.services.mikrotik_service import get_mikrotik_system_info, reboot_mikrotik
...
class ChatbotService:
    ...
    def __init__(self):
        self.proxmox_service: Optional[ProxmoxService] = None
    ...
    async def _handle_get_status(self, target: Optional[str], parameters: Dict[str, Any]) -> str:
    ...
    async def _handle_analyze_service(self, target: Optional[str], parameters: Dict[str, Any]) -> str:
    ...
    async def _handle_run_playbook(self, target: Optional[str], parameters: Dict[str, Any]) -> str:
    ...
    async def _handle_get_logs(self, target: Optional[str], parameters: Dict[str, Any]) -> str:
    ...
    async def _handle_restart_service(self, target: Optional[str], parameters: Dict[str, Any]) -> str:
    ...
    async def _handle_list_vms(self, target: Optional[str], parameters: Dict[str, Any]) -> str:
        ...
        try:
            ...
            return (
                f"🖥️ **Список VM/контейнеров** ({len(vms)} всего):\n\n" +
                "\n".join(vm_lines)
            )
    ...
    async def _handle_vm_action(self, target: Optional[str], parameters: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if action == "start":
                result = proxmox.start_vm(node, vmid)
                return f"▶️ VM **{vmid}** запускается на ноде `{node}`"
            elif action == "stop":
                result = proxmox.stop_vm(node, vmid)
                return f"⏹️ VM **{vmid}** останавливается на ноде `{node}`"
            elif action == "reboot":
                result = proxmox.reboot_vm(node, vmid)
    ...
    async def _handle_mikrotik_info(self, target: Optional[str], parameters: Dict[str, Any]) -> str:
        """Обработчик информации о MikroTik."""
        return await self._handle_get_status("mikrotik", parameters)
    
    async def _handle_mikrotik_reboot(self, target: Optional[str], parameters: Dict[str, Any]) -> str:
    ...
    async def _handle_help(self, target: Optional[str], parameters: Dict[str, Any]) -> str:
        """Обработчик справки."""
        return (
    ...
    async def _handle_unknown(self, target: Optional[str], parameters: Dict[str, Any]) -> str:
