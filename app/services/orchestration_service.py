...
class RemediationCycle:
    ...
    def add_step(self, step_name: StepName, details: dict[str, Any] = None):
        ...
        self.steps[step_name] = {"status": StepStatus.PENDING, "details": details or {}}
...
class OrchestrationService:
    ...
    def _create_cycle(self, device_type: str, device_host: str, problem: str) -> RemediationCycle:
    ...
    async def _verify_fix(
        self, device_type: str, device_host: str, original_problem: str, wait_seconds: int = 60
    ) -> bool:
        """
        ...
        try:
            process = await asyncio.create_subprocess_shell(
                f"ping -c 3 -W 5 {device_host}", stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
            )
    ...
    async def execute_and_verify_remediation(
        ...
        await self.telegram_service.send_message(f"▶️ *Цикл {cycle.id}*: Выполняется плейбук `{playbook_name}`...")

        try:
            success, output = await asyncio.to_thread(
                self.ansible_service.run_playbook, playbook_path, device_type, device_host
            )
        except Exception as e:
        ...
        await self.telegram_service.send_message(f"✅ *Цикл {cycle.id}*: Плейбук `{playbook_name}` выполнен.")

        # --- Шаг 2: Пост-анализ результатов ---
        cycle.add_step(StepName.POST_ANALYSIS)

        await self.telegram_service.send_message(f"🔍 *Цикл {cycle.id}*: Анализ результатов выполнения...")
        ...
        if analysis["status"] == "FAILURE":
            ...
            return {"cycle_id": cycle.id, "status": "failure", "step": "post_analysis", "message": analysis["reason"]}
    ...
    def get_cycle_status(self, cycle_id: str) -> dict[str, Any] | None:
        ...
        return {
            ...
            "steps": {
                name.value: {"status": step["status"].value, "details": step["details"]}
