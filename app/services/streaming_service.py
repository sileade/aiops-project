...
class StreamingService:
    ...
    async def publish_log(self, log: LogEntry) -> str | None:
        """
        ...
        try:
            entry_id = await self.redis.xadd(
                self.STREAM_KEY, log.to_dict(), maxlen=self.MAX_STREAM_LENGTH, approximate=True
            )
            return entry_id
        except Exception as e:
    ...
    async def publish_batch(self, logs: list[LogEntry]) -> int:
        ...
        for log in logs:
            pipeline.xadd(self.STREAM_KEY, log.to_dict(), maxlen=self.MAX_STREAM_LENGTH, approximate=True)
    ...
    async def start_consumer(self, consumer_name: str = "consumer-1", handler: Callable | None = None):
        """
    ...
    async def get_stream_info(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "groups": [
                    {"name": g.get("name"), "consumers": g.get("consumers"), "pending": g.get("pending")}
                    for g in groups
                ],
            }
        except Exception as e:
