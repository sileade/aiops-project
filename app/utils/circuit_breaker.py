        ...
        async with self._lock:
            ...
            if self._state.state == CircuitState.HALF_OPEN:
                ...
                if self._state.failure_count >= self.config.failure_threshold:
                    self._state.state = CircuitState.OPEN
                    logger.warning(
                        f"Circuit breaker '{self.name}' OPEN after {self._state.failure_count} failures"
                    )
...
ollama_breaker = CircuitBreaker("ollama", CircuitBreakerConfig(
    failure_threshold=5,
    timeout=30.0,
    success_threshold=1
))

elasticsearch_breaker = CircuitBreaker("elasticsearch", CircuitBreakerConfig(
    failure_threshold=5,
    timeout=30.0
))

prometheus_breaker = CircuitBreaker("prometheus", CircuitBreakerConfig(
    failure_threshold=5,
    timeout=30.0
))
